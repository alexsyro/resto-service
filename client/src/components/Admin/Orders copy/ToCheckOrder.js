  ...
  return (
    <li>
      <button onClick={() => finishOrder()} className='uk-button uk-button-primary'>
        {' '}
        перевести в статус подтвержденного
      </button>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
        ...
        <span>
          Дата бронирования: {`${order.timeFormat.day}  ${order.timeFormat.month}, ${order.timeFormat.year}`}
        </span>
      ...
      <hr />
      <button className='uk-button uk-button-small' onClick={() => setMenuList((prev) => !prev)}>
        {' '}
        {menuList ? 'Скрыть' : 'Вывести'} список заказанных блюд
      </button>
      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
        {menuList && (
          <ol>
            {order.OrderPositions.map((position) => {
              return (
                <li key={position.id}>
                  ...
                  <button onClick={() => cancelPosition(position.id)} className='uk-button uk-button-primary'>
                    {' '}
                    Удалить блюдо
                  </button>
      ...
      <button onClick={() => cancelOrder()} className='uk-button uk-button-primary'>
        {' '}
        Отменить заказ
      </button>
      <Link to={`/orders/${order.id}`} className='uk-button uk-button-default'>
        Изменить резервирование столика
      </Link>
    </li>
  );
}
